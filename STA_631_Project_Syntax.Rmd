...
batting_data <- batting_data %>%
  select(c("name", "team", "position", "game", "at_bat", "bat_avg"))
  
person_data$name <- paste(substr(person_data$nameFirst,1,1),person_data$nameLast)

person_data <- person_data %>% select(c("name","birthYear","birthMonth", "birthCountry","birthState","weight","height", "bats"))
```

```{r combined_data}
dfList <- list(batting_data, person_data)
batting_person <- Reduce(function(x, y) merge(x, y), dfList) %>% 
  filter(at_bat > 30)
```
